...
function onFocus(e) {
    ...
    if(eventFlag) {e.currentTarget.removeEventListener('focusout', outfocus);eventFlag=false;}
    else {e.currentTarget.addEventListener('focusout',outfocus);eventFlag=true;}
    e.currentTarget.setCustomValidity('');
}
function outfocus(e) {
    e.currentTarget.removeEventListener('focusout', outfocus);    
    e.currentTarget.setCustomValidity('');
    if(!e.currentTarget.checkValidity()) {
        ...
        setTimeout(()=> {
            // this.addEventListener('blur',outfocus).bind(e.currentTarget);
            reapply.bind(this);
        },1000);
    } else {
...
function reapply(e) {
    e.currentTarget.addEventListener('blur',outfocus);
}   
function onType(e) {
